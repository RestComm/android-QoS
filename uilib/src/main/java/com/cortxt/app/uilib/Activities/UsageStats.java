...
import com.cortxt.app.uilib.Activities.CustomViews.UsageAdapter;
import com.cortxt.app.uilib.R;
import com.cortxt.app.utillib.Utils.LoggerUtil;
import com.cortxt.app.utillib.Utils.Stats;
import com.cortxt.app.utillib.DataObjects.beans.CPUStatBean;
import com.cortxt.app.utillib.DataObjects.beans.DataStatsBean;
import com.cortxt.app.utillib.DataObjects.beans.MemoryStatBean;
...
//		});
	...
	private void executeTop() {
		...
		try {
			...
			if (p == null) {
				LoggerUtil.logToFile(LoggerUtil.Level.WARNING, TAG, "executeTop", "Requested Program cannot be executed.");
			...
			LoggerUtil.logToFile(LoggerUtil.Level.WARNING, TAG, "executeTop", "total number of running processes" + listOfData.size());
			
		} catch (IOException e) {
			LoggerUtil.logToFile(LoggerUtil.Level.WARNING, TAG, "executeTop", "Requested Program cannot be executed.", e);
			...
			try {
				...
				LoggerUtil.logToFile(LoggerUtil.Level.WARNING, TAG, "executeTop", "error in closing and destroying top process.", e);
			}
		}
	}
	...
	private static void destroyProcess(Process process) {
		try {
			...
			LoggerUtil.logToFile(LoggerUtil.Level.ERROR, TAG, "destroyProcess", "Illegal state exception occurred while destroying the process.", e);
			process.destroy();
		}
	}
