package com.restcomm.app.qoslib.UtilsOld;
...
public class AccessPointHistory {
	...
	private MainService mContext = null;
	
	public static final String TAG = AccessPointHistory.class.getSimpleName();

	public AccessPointHistory (MainService context)
	{
		mContext = context;
...
//					wifiId = wifiInfo.getBSSID();
					...
					if (allow > 0 && mContext.getUsageLimits().getUsageProfile () > UsageLimits.MINIMAL)
					{
						...
						mContext.handler.postDelayed(new Runnable() {
							...
							public void run() {
								...
								{
									LoggerUtil.logToFile(LoggerUtil.Level.DEBUG, TAG, "beginEvent", "Wifi connected < 10 minutes, ignoring");
									return;
								}
			...
			{
				LoggerUtil.logToFile(LoggerUtil.Level.ERROR, TAG, "beginEvent:wifiManager.getConnectionInfo()", "exception", e);
			}
			break;
